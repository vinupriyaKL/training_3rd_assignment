...
public ResponseEntity<?> createReminder(HttpSession session, @RequestBody Reminder reminder){
...
public ResponseEntity<?> deleteReminder(@PathVariable("id") int reminderId,HttpSession session) throws ReminderNotFoundException {
...
else if(reminderService.deleteReminder(reminderId)==false) {
	return new ResponseEntity<>("Reminder already exists", HttpStatus.NOT_FOUND);

}
	else {
	return new ResponseEntity<>("Reminder already exists", HttpStatus.CONFLICT);
}
...
public ResponseEntity<?> updateReminder(@RequestBody Reminder reminderWithNewValue,@PathVariable("id") int reminderId, HttpSession session) throws ReminderNotFoundException {
	...
	if (reminderService.updateReminder(reminderWithNewValue, reminderId)!=null) {
		return new ResponseEntity<>("Reminder created successfully", HttpStatus.OK);
	} 
	else if(reminderService.updateReminder(reminderWithNewValue, reminderId)==null) {
...
@GetMapping("/reminder")
public ResponseEntity<?> returnReminderByUser( HttpSession session){
		...
		else if(reminderService.getAllReminderByUserId(session.getAttribute("loggedInUserId").toString())!=null) {
			return new ResponseEntity<String>("OK",HttpStatus.OK);

		}
		else {
			return new ResponseEntity<String>("rem",HttpStatus.NOT_FOUND);
...
public ResponseEntity<?> returnReminderById(@PathVariable("id") int reminderId,HttpSession session) throws ReminderNotFoundException{
		...
		else if(session.getAttribute("loggedInUserId")!=null && reminderService.getReminderById(reminderId)!=null) {
		 Reminder remiderByid= reminderService.getReminderById(reminderId);
			return new ResponseEntity<Reminder>(remiderByid,HttpStatus.OK);
