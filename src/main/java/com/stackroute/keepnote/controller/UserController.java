...
public ResponseEntity<?> registerUser(@RequestBody User user, HttpSession session) throws UserAlreadyExistException{
	try
	{
	...
	catch(UserAlreadyExistException item)
	{
...
@PutMapping("/user/{id}")
public ResponseEntity<?> updateUser(@RequestBody User userWithNewValue,@PathVariable("id") String userId, HttpSession session) throws Exception {
	...
	if (userService.updateUser(userWithNewValue, userId)!=null) {
		return new ResponseEntity<>("User created successfully", HttpStatus.OK);
	} 
	else if(userService.updateUser(userWithNewValue, userId)==null) {
...
public ResponseEntity<?> deleteUser(@PathVariable("id") String userId, HttpSession session) throws UserNotFoundException {
	...
	else if(userService.deleteUser(userId)==false) {
...
@GetMapping("/user/{id}")
public ResponseEntity<?> getUserById(@PathVariable("id") String userId, HttpSession session) throws UserNotFoundException {

	User userChcek=userService.getUserById(userId);

	if(session.getAttribute("loggedInUserId")==null) {
		return new ResponseEntity<String>("please do login first",HttpStatus.UNAUTHORIZED);

	}
	 else if(userChcek!=null) {
